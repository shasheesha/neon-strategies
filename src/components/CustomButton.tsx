...
const CustomButton: React.FC<Props> = ({submit, btnName, classNames, arrowClassNames, withArrow, onClick}) => {

    return(
        <>
        <button type={submit === true ? "submit" : "button"} onClick={onClick} className={`group relative flex flex-row gap-5 justify-center items-center font-medium rounded-full text-nowrap transition ease-in-out duration-300 delay-100 ${classNames}`}>
            <span>{btnName}</span>
